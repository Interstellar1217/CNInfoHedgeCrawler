...
async def random_delay_async() -> None:
    """
    异步随机延时

    需求：在异步环境中使用，避免阻塞事件循环
    """
    delay = config.get_random_delay()
    logger.debug(f"异步随机延时 {delay:.2f} 秒")
...
def extract_announcement_id_from_url(url: str) -> Optional[str]:
    ...
    需求：获取公告详情页中的唯一标识符
    实现思路：解析 URL 参数中的 announcementId
    ...
    try:
        ...
        logger.error(f"从 URL 提取公告 ID失败 {url}: {e}")
        return None
...
def generate_filename(title: str, announcement_id: str, file_type: str = "pdf") -> str:
    ...
    需求：根据公告标题和 ID 生成唯一且安全的文件名
    实现思路：清理特殊字符，使用公告 ID 保证唯一性

    Args:
        title: 公告标题
        announcement_id: 公告 ID
        file_type: 文件类型（pdf, html 等）
...
def load_metadata(file_path: str) -> list:
    ...
    需求：从 JSON 文件加载已保存的元数据
    实现思路：逐行读取 JSON 文件，解析每条记录
